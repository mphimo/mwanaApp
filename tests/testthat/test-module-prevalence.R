...
testthat::test_that(desc = "Module works well to estimate weighted-WFHZ prevalence from survey", code = {
  ...
  weighted_pop <- sub(
    "1",
    "",
    stringr::str_extract(glued_results, "\\d{7}(?:)")
  )
...
testthat::test_that(desc = "Module works well to estimate unweighted-WFHZ prevalence from survey", code = {
  ...
  ### Capture prevalence results of Nampula Province, Rural Strata ----
  glued_results <- app$get_js(js_values)[[3]]
  pop <- sub("1", "", stringr::str_extract(glued_results, "\\d{4}"))
  gam_prev <- stringr::str_extract(glued_results, "\\d{1}\\.\\d")
...
testthat::test_that(desc = "Module works well to estimate weighted-MUAC prevalence from survey", code = {
  ...
  weighted_pop <- sub(
    "1",
    "",
    stringr::str_extract(js_result[[3]], "\\d{7}(?:)")
  )
...
testthat::test_that(desc = "Module works well to estimate unweighted-combined prevalence from survey", code = {
  ...
  glued_results <- app$get_js(js_values)[[3]]
  pop <- sub("1", "", stringr::str_extract(glued_results, "\\d{4}"))
  prev <- stringr::str_extract(glued_results, "\\d{2}\\.\\d")
...
testthat::test_that(desc = "Module works well to estimate prevalence from screening", code = {
  ...
  ### Select source of data ----
  app$set_inputs(
    `prevalence-source` = "screening",
    wait_ = TRUE,
    timeout_ = 40000
  )

  ### Select the method ----
  app$set_inputs(`prevalence-has_age` = "yes", wait_ = TRUE, timeout_ = 40000)
  ...
  #### Prevalences ----
  prev_unit_a <- stringr::str_extract_all(glued_results_unit_a, "\\d\\.\\d")[[
    1
  ]]
